...
An adversarial QA pass for any codebase, in any language. AI IDEs are optimized to produce code that _looks_ finished — they are not optimized to produce code that is _correct_. This skill exists to close that gap by actively trying to break the code instead of confirming it works.
...
- **Confidence:** _(omit if fully confirmed in-scope; include "Needs Verification" if it depends on code outside the audited scope)_
- **Status:** Open
