...
1. **Render N labeled variants into ONE artifact.** Vary the parameter(s) across a
   ...
   labeled _sequence_ (label card or burned-in overlay before/over each clip) so the
...
- **Cheapest judge that can see the failure.** Frames-through an image VLM can judge
  spatial things (layout, color, crop); only reach for a true _video_ model when the
...
- The model's pick is an _opinion_, not ground truth — anchor it, and sanity-check the
