...
**Before reporting any design "done": render it and have a _separate_ judge critique the image** (not the code, not self-grading) — see design-spatial §1. Blind generation can't see its own collisions; this applies to all design output, not just spatial work.
...
tells the user _exactly_ what the knob does and builds trust that there's real math
under the hood.
...
- This composes with the model+cost annotation above: the equation says _what_ it
  computes, the model/cost line says _what ran it and for how much_.
...
preferably the _same button_, label-flipped when everything is already selected
...
the page, to get back. The cost is silent — it only bites _after_ they've committed to
the wrong set.
...
- The deselect must reach the **same scope** the select-all did (all _shown_, all
  _filtered_, all _on this page_) — don't let "Select all" grab 500 but "Clear" only
