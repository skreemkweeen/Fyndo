...
function __audit(opts) {
    ...
    opts.contentSelector
      ...
      : [
          ...document.querySelectorAll(
            'h1,h2,h3,h4,p,li,img,svg,button,a,input,label,figure,blockquote,[class*="card"],[class*="tile"],[class*="btn"],[class*="hero"],[class*="banner"],[class*="panel"],[class*="badge"],[class*="chip"],[class*="thumb"],[class*="avatar"]',
          ),
        ]
  )
    .filter(vis)
    .map(R)
    .filter((it) => it.w >= 8 && it.h >= 8 && it.w < innerWidth * 1.5);
  ...
  for (let i = 0; i < CONTENT.length; i++)
    for (let j = i + 1; j < CONTENT.length; j++) {
      ...
      const ix = Math.max(0, Math.min(a.r, b.r) - Math.max(a.x, b.x)),
        iy = Math.max(0, Math.min(a.b, b.b) - Math.max(a.y, b.y));
      if (ix * iy <= 0) continue;
      if ((ix * iy) / Math.min(a.w * a.h, b.w * b.h) > 0.12)
        collisions.push([a, b]);
    }
  ...
  let nearmiss = [],
    guides = [];
  if (opts.align) {
    const els = [...document.querySelectorAll(opts.align)]
      .filter(vis)
      .map(R)
      .sort((a, b) => a.x - b.x);
    let cur = [];
    const cl = [];
    els.forEach((e) => {
      if (cur.length && e.x - cur[cur.length - 1].x > 11) {
        cl.push(cur);
        cur = [];
      }
      cur.push(e);
    ...
    cl.forEach((c) => {
      ...
      const xs = c.map((e) => e.x).sort((a, b) => a - b),
        med = xs[(xs.length / 2) | 0],
        spread = xs[xs.length - 1] - xs[0];
      guides.push(med);
      if (spread <= 0.8 || spread > 10) return;
      c.forEach((e) => {
        if (Math.abs(e.x - med) > 0.8)
          nearmiss.push({
            x: e.x,
            med,
            y: e.y,
  ...
  if (opts.space) {
    const cont = document.querySelector(opts.space);
    if (cont) {
      ...
      for (let i = 0; i < kids.length - 1; i++) {
        const g = kids[i + 1].y - kids[i].b;
        if (g > -6 && g < 400)
          gaps.push({ g: Math.round(g), a: kids[i], b: kids[i + 1] });
      }
      if (gaps.length >= 2) {
        const v = gaps.map((o) => o.g),
          mn = v.reduce((s, x) => s + x, 0) / v.length,
          sd = Math.sqrt(v.reduce((s, x) => s + (x - mn) ** 2, 0) / v.length);
        spacing = {
          gaps,
          cov: mn ? Math.abs(sd / mn) : 0,
          mean: mn,
          outliers: gaps.filter(
            (o) => Math.abs(o.g - mn) > Math.max(6, Math.abs(mn) * 0.5),
          ),
        };
      }
    }
  ...
  [
    ...document.querySelectorAll(
      "h1,h2,h3,h4,h5,h6,p,a,span,li,button,label,td,th,figcaption,small,strong,em",
    ),
  ]
    .filter(vis)
    .forEach((el) => {
  ...
  const tap = [
    ...document.querySelectorAll(
      'a,button,[role="button"],input:not([type="hidden"]),select,textarea,[onclick]',
    ),
  ]
    .filter(vis)
    .map(R)
    .filter((it) => it.w < 44 || it.h < 44);
  ...
  CONTENT.forEach((it) => {
    const w = it.w * it.h * dens(it.el);
    W += w;
    Mx += w * (it.cx + scrollX);
    My += w * (it.cy + scrollY);
  });
  const balance = W
    ? { cx: Mx / W / pw, cy: My / W / ph }
    : { cx: 0.5, cy: 0.46 };
  ...
  if (DRAW) {
    ...
    collisions.forEach(([a, b]) =>
      [a, b].forEach((it) =>
        add("rect", {
          ...
          width: it.w,
          height: it.h,
          fill: COL.collision + "22",
          stroke: COL.collision,
          "stroke-width": 2.5,
        }),
      ),
    );
    ...
    spacing.gaps.forEach((o) => {
      const bad = spacing.outliers.includes(o),
        x = Math.min(o.a.x, o.b.x) - 16;
      add("line", {
        x1: x,
        y1: o.a.b,
        x2: x,
        y2: o.b.y,
        stroke: bad ? COL.collision : COL.spacing,
        "stroke-width": bad ? 3.5 : 2,
      });
      add("text", {
        x: x - 7,
        y: (o.a.b + o.b.y) / 2 + 4,
        fill: bad ? COL.collision : COL.spacing,
        "font-family": "monospace",
        "font-size": 13,
        "text-anchor": "end",
        _t: o.g,
    ...
    tap.forEach((it) => {
      ...
      add("text", {
        ...
        "font-family": "monospace",
        "font-size": 12,
        _t: Math.round(it.w) + "×" + Math.round(it.h),
      });
    });
  ...
  return {
    ...
    gates: {
      ...
      contrast_fails: contrast.length,
      tap_too_small: tap.length,
    },
    ...
    signals: {
      balance_offset: +balance.off.toFixed(3),
      balance_centroid: [+balance.cx.toFixed(3), +balance.cy.toFixed(3)],
      align_nearmiss: nearmiss.length,
      spacing_cov: +spacing.cov.toFixed(2),
    },
    contrast_detail: contrast.map((c) => ({ ratio: c.ratio, large: c.large })),
    note:
      "These are HEURISTICS. (1) LOOK at the annotated screenshot — a flag marks WHERE to look, never a verdict. " +
...
function __clearAudit() {
  const o = document.getElementById("__auditOverlay");
  if (o) o.remove();
}
