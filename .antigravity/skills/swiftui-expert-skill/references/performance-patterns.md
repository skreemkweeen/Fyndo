...
**Avoid storing frequently-changing values in the environment.** Every write to _any_ environment key forces every view that reads _any_ key in that subtree to be checked. So a high-frequency value (scroll offset, window/container size, drag translation, per-frame animation progress, timer ticks, hover location) flowing into an `@Entry` or `.environment(\.key, value)` becomes an invalidation storm.
