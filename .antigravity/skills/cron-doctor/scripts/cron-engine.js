...
function parseField(raw, fieldDef, fieldIndex) {
  ...
  if (fieldDef.key === "dow" && /L$/i.test(trimmed)) {
    ...
    if (!m)
      throw new CronError(
        `Invalid "L" syntax in day-of-week: "${trimmed}"`,
        fieldIndex,
      );
    let dowNum = parseSingleNum(m[1], fieldDef, fieldIndex);
    if (dowNum === 7) dowNum = 0;
...
function parseSingleNum(token, fieldDef, fieldIndex) {
  ...
  throw new CronError(
    `Invalid value "${value}" in ${fieldDef.name}`,
    fieldIndex,
  );
}
...
function parseItem(item, fieldDef, fieldIndex, values) {
  ...
  if (t.includes("/")) {
    ...
    if (stepParts.length !== 2)
      throw new CronError(
        `Invalid step syntax "${t}" in ${fieldDef.name}`,
        fieldIndex,
      );
    const [base, stepStr] = stepParts;
    if (!/^\d+$/.test(stepStr))
      throw new CronError(
        `Invalid step "${stepStr}" in ${fieldDef.name}`,
        fieldIndex,
      );
...
function describeFieldMonth(field) {
  ...
  return {
    restricted: true,
    text: "in " + listJoin(vals.map((v) => capitalize(MONTH_NAMES[v - 1]))),
  };
}
...
function describeFieldDom(field) {
  ...
  if (field.special) {
    if (field.special.kind === "domLast") {
      return {
        text:
          field.special.offset === 0
            ? "the last day of the month"
            : `the last day of the month minus ${field.special.offset} days`,
      };
    }
...
function describeFieldDow(field) {
  ...
  if (field.special) {
    if (field.special.kind === "hash") {
      return {
        text: `the ${ordinal(field.special.nth)} ${capitalize(DAY_NAMES[field.special.dow])} of the month`,
      };
    }
    if (field.special.kind === "dowLast") {
      return {
        text: `the last ${capitalize(DAY_NAMES[field.special.dow])} of the month`,
      };
    }
  }
  const vals = [...(field.values || [])].sort((a, b) => a - b);
...
function matchDomSpecial(d, special) {
  ...
  if (special.kind === "weekday") {
    return (
      d.getDate() === nearestWeekday(d.getFullYear(), d.getMonth(), special.day)
    );
  }
  return false;
}
...
function validate(expr) {
  ...
  if (parsed.domRestricted && parsed.dowRestricted) {
    warnings.push({
      level: "high",
      message:
        "Both day-of-month and day-of-week are restricted. Cron uses OR semantics for these fields — the job will run when EITHER matches, not both. This is a common source of bugs.",
    });
  }
  ...
  if (
    ...
    parsed.month.values &&
    ![...parsed.month.values].every((m) => m === 2)
  ) {
    // skip
  } else if (
    parsed.domRestricted &&
    !parsed.dom.special &&
    domValues.includes(31)
  ) {
    ...
    if (restrictedMonths) {
      const problemMonths = monthValues.filter(
        (m) => !monthsWith31.includes(m),
      );
      ...
      observations.push({
        level: "info",
        message:
          "Day 31 will only match in months with 31 days (7 of 12 months). The job effectively skips Feb, Apr, Jun, Sep, and Nov.",
      });
    }
  }
  ...
  if (parsed.parts[0] === "*" && parsed.parts[1] === "*") {
    observations.push({
      level: "info",
      message:
        "This expression runs every minute. For production jobs, consider if this frequency is intentional.",
    });
  }
  ...
  if (parsed.domRestricted && !parsed.dom.special) {
    ...
    if (domVals.includes(29) && monthVals.length === 1 && monthVals[0] === 2) {
      warnings.push({
        level: "medium",
        message:
          "February 29th only occurs in leap years. This job will not run at all in non-leap years (3 out of every 4 years).",
      });
    }
  }
  ...
  if (parsed.parts[0] === "0" && parsed.parts[1] === "0") {
    suggestions.push({
      level: "info",
      message:
        "Midnight (00:00) is a common schedule and many systems have concurrent job spikes at this time. Consider offsetting to a few minutes past midnight (e.g., 02 0 * * *) to avoid resource contention.",
    });
  }
