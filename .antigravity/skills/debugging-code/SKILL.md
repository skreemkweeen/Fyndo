...
A debugger lets you _observe_ what _does_ happen: actual values, actual path, actual state.
When that diverges from what _should_ happen, you've found your bug.

**Two strikes, rethink.** If two hypotheses fail at the same location, your mental model is wrong.
Re-read the code, form a _completely different_ theory with different breakpoints.
...
Before setting a breakpoint: _"I believe the bug is in X because Y."_ A good hypothesis is falsifiable — your next
...
- Set where the problem _begins_, not where it _manifests_
...
Conditional breakpoints as runtime assertions — stop the _moment_ something goes wrong:
