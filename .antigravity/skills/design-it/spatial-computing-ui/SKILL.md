...
1. **Z-Space Hierarchy**: Not just drop shadows, but actual 3D distance. Modals float 10px _in front of_ the main window.
...
- React Native's core cannot do background blur. You _must_ install `@react-native-community/blur`.
...
- On Android 12+, use `RenderEffect.createBlurEffect` applied to the `graphicsLayer` to blur the UI _behind_ the compose element.
