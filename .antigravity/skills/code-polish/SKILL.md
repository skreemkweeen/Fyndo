...
Comments and non-semantic cleanup are the job. Logic is never the job. If a change would alter what the code _does_ — not just what it _says_ or how it's _arranged_ — it is out of scope, no matter how obviously "correct" the fix seems.
...
| **Valuable but informal** | `// careful, this breaks if you call it twice, learned that the hard way`   | Preserve the _information_, rewrite the _tone_. Never delete real warnings just because the phrasing is casual                                                       |
...
- **Explain why, not what.** The code already shows _what_ it does; a comment earns its place by explaining intent, tradeoffs, or non-obvious constraints.
...
- Comment quality is bounded by how well the code's actual intent can be inferred from context. If the "why" genuinely isn't recoverable from the file (no domain knowledge, no commit history, no ticket references available), the honest output is a comment describing _what_, not a confident but invented _why_.
