...
_Scan this first — the most visible risk and the one that determines whether the suite
is maintainable at all._
...
_Brittle tests break on refactors that do not change observable behavior — they test
implementation, not contracts._
...
_Mock Abuse produces tests that pass regardless of whether the real behavior is correct.
Scan this separately from brittleness — over-mocking is often the cause of brittleness,
but it is a distinct problem worth its own finding._
