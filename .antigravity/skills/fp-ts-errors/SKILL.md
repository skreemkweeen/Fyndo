...
const formValidation = E.getApplicativeValidation(
  NEA.getSemigroup<FieldError>(),
);
...
const bulkProcess = <T>(
  ...
  pipe(
    ...
    T.map((results) => ({
      succeeded: results
        .filter(
          (r): r is { type: "succeeded"; result: T } => r.type === "succeeded",
        )
        .map((r) => r.result),
      failed: results
        .filter(
          (r): r is { type: "failed"; id: string; error: string } =>
            r.type === "failed",
        )
