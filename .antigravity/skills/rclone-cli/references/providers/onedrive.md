...
3. Under `manage` select `Certificates & secrets`, click `New client secret`.
   ...
   Copy and keep that secret _Value_ for later use (you _won't_ be able to see
...
2. Ensure that the application has the appropriate permissions and they are
   assigned as _Application Permissions_
3. Configure the remote, ensuring that _Client ID_ and _Client Secret_ are
   entered correctly.
4. In the _Advanced Config_ section, enter `true` for `client_credentials` and
...
application the necessary Microsoft Graph _Application permissions_
...
but it needs to know _which user or drive_ you want. Supply a specific
...
anyone with the _Client ID_ and _Client Secret_ can access your
...
This will work if you are copying between two OneDrive _Personal_ drives AND the files to
copy are already shared between them. Additionally, it should also function for a user who
has access permissions both between Onedrive for _business_ and _SharePoint_ under the _same
tenant_, and between _SharePoint_ and another _SharePoint_ under the _same tenant_. In other
...
_Below are the steps for normal users to disable versioning. If you don't see
the "No Versioning" option, make sure the above requirements are met._
