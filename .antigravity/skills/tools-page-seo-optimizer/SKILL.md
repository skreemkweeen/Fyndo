...
<meta
  name="description"
  content="{{ tool.meta_description | default(tool.tagline) }}"
/>
```
...
meta_description:
  "Generate optimized title tags and meta descriptions with live character
...
faqs:
  - q: "Does Google always use my meta description?"
    a:
      "No — Google rewrites descriptions ~63% of the time. Write them anyway for
...
<section>
  <h2>Related Tools</h2>
  {% for slug in tool.related_tools %} {% set rel = get_tool(slug) %}
...
<p class="tool-byline">
  ...
  <time datetime="{{ tool.updated_at }}">
    · Updated {{ tool.updated_at | date }}</time
  >
</p>
```
...
<p>
  Use our <a href="/tools/meta-tag-generator">meta tag generator</a> to preview
  how your title and description appear in Google results before publishing.
</p>
