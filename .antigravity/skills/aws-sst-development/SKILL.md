...
_any_ raw Pulumi `aws.*` resource for the long tail. This skill encodes a
...
from memory. The patterns here are the _how_; the docs are the _what_.
...
Always read the relevant reference before editing — they carry the _why_ behind
...
1. **`sst.config.ts`** — the app name, `home`, providers/region, `defaultTags`,
   ...
   which `run()` imports `infra/` modules. The import order _is_ the dependency
   order; respect it.
...
- **Migrating a resource between Pulumi _types_ should default to two PRs** —
...
- **SSM Parameter Store as the out-of-graph contract** under a
  ...
  Pulumi graph (CI scripts, sibling apps, operators). For _same-app_ Lambdas,
...
- **Source-level Vitest tests** on every infra module — a lightweight,
  house-style regression net asserting on the _source text_ (resource names,
  index shapes, IAM scopes). It's a deliberate choice, not an SST limit: Pulumi
  _does_ support runtime mocks (`@pulumi/pulumi/runtime`) for behavioral graph
...
3. **Author** the resource/module following `references/authoring.md`. Match the
   ...
   _why_ heavily, and a terse one-liner in a heavily-annotated file reads as a
   regression.
