...
const fetchUser = (id: string): TE.TaskEither<ApiError, User> =>
  TE.tryCatch(
    async () => {
      ...
      if (!res.ok)
        throw { _tag: "NetworkError", cause: new Error(`HTTP ${res.status}`) };
      return res.json();
    },
    (e): ApiError =>
...
const updateUser = (id: string, data: UpdateUserDto) =>
  ...
  if (error instanceof Error) {
    if ("code" in error) {
      ...
      if (error.code === "EACCES")
        return { _tag: "PermissionDenied", path: filePath };
    }
  }
  ...
  pipe(
    ...
    TE.chain((content) =>
      TE.tryCatch(
        ...
        (e): { _tag: "ParseError"; cause: unknown } => ({
          _tag: "ParseError",
          cause: e,
        }),
      ),
...
pipe(
  fetchUser(userId),
  TE.tapError((error) =>
    TE.fromIO(() => console.error(`Failed: ${error.message}`)),
  ),
);
