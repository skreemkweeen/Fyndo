...
- Self-report only, by design: it does not auto-track apps or screens (an auto-tracker knows what was open, not _why_ the time was spent).
...
- On claude.ai, Notion's write tools default to _needs approval_ — the first write pops an approval prompt; this is expected, not a hang.
