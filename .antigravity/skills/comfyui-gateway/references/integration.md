...
```json
{
  "workflows": [
    {
      "id": "sdxl_realism_v1",
      "name": "SDXL Realism v1",
      "description": "..."
    },
    {
      "id": "sprite_transparent_bg",
      "name": "Sprite Transparent BG",
      "description": "..."
    }
  ],
  "maxSize": 2048,
...
const GATEWAY_URL =
  Deno.env.get("COMFYUI_GATEWAY_URL") ?? "http://localhost:3000";
...
export class ComfyUIGateway {
  ...
  async listOutputs(
    ...
    Array<{ filename: string; size: number; sha256: string; url: string }>
  > {
    const data = await this.request<{
      files: Array<{
        filename: string;
...
async function handleSuccess(payload) {
  ...
  if (payload.result?.outputs?.length > 0) {
    ...
    console.log(
      `  - ${output.filename}: ${output.size} bytes, URL: ${output.url}`,
    );
