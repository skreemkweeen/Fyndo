...
function extractComponents(indexFile) {
  ...
  for (const line of src.split("\n")) {
    ...
    if (m) {
      ...
      if (
        /^(types|utils|index|State|hooks|colors|layout-types|MaterialSymbols)/.test(
          name,
        )
      )
        continue;
...
function extractModifiers(modifiersFile) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    const m = line.match(
      /^export declare (?:const|function) ([a-zA-Z_][a-zA-Z0-9_]*)/,
    );
    if (!m) continue;
    const name = m[1];
...
// Universal
const universalComponents = extractComponents(
  path.join(buildRoot, "universal", "index.d.ts"),
);

// Swift-UI (iOS only)
const swiftuiComponents = extractComponents(
  path.join(buildRoot, "swift-ui", "index.d.ts"),
);
...
const composeComponents = extractComponents(
  path.join(buildRoot, "jetpack-compose", "index.d.ts"),
);
