...
- Naive RAG tool: _Basic retrieval as a single tool for the RouterAgent_
- Hierarchical RAG: _LLM-created filters and search parameters as a sub-agent tool_
- Vector DB memory: _Store and retrieve facts across sessions using Weaviate_
- Query Agent: _Pre-built agentic RAG service by Weaviate_
- Elysia: _Open source agentic framework with built-in query tool_
