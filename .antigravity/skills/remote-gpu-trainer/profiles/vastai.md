...
GPUs free) while the stopped instance is stuck in `Scheduling` waiting for _its_ GPU (VAST3).
...
"each open internal port (such as 22 or 8080 etc) is mapped to a _random_ external port" read from the
...
direct + a resumable loop (`references/gotchas_universal.md` U12). For a big _inbound_ dataset, prefer
...
egress option), but a given offer **can** charge per-GB in _both_ directions — read the per-offer bandwidth
...
built-in HF mirror. The HF-unreachable problem still exists at the _workload_ level from some hosts, but the
...
- **Bandwidth bills in EVERY state.** Charged "for every byte sent or received... regardless of what state it
  is in" — so even a transfer to/from a _stopped_ instance (cloud sync) accrues host-set bandwidth cost (§3).
...
- **VAST3 — paused/stopped instance stuck in `Scheduling` though the machine shows "available."** Symptom:
  ...
  indefinitely while _other_ GPUs on the host stay free. "If stuck >30 s, GPU likely rented by another user."
...
- **VAST8 — bulk `scp` over the default SSH stalls / crawls.** Symptom: a multi-GB result copy over the
  ...
  `direct_port_count>=1`) and use that endpoint for `scp`/`vastai copy`; for big _inbound_ data prefer
...
- **Inspect the live state machine without SSH:** `vastai show instance <id>` (or the API) — compare
  `actual_status` (where the container _is_), `intended_status` (where it _should_ be), `cur_state` (contract/
...
- **Detect a stuck download inside the box:** `du -sh ~/.cache/huggingface/hub` over time (no growth = stalled
  ...
  in `gotchas_universal.md` (HF). A fat-image stall _before_ SSH is visible only via `vastai show logs`.
