...
channel and teardown is manual** — every other platform profile is a _diff_ against this baseline.
...
- **Entry point:** `ssh user@host` — key-based, fronted by an `~/.ssh/config` alias so the rest of
  the workflow says `ssh gpu-box`. There is **no platform API, console, or CLI** — SSH is the _only_
...
## 2. STORAGE MODEL _(the survival matrix — principle #4)_

The box gives **one persistent disk that is yours to manage** — no shared FS, no platform quota
service, no automatic reclamation. _Measure, never assume:_ run `df -h && df -i <mount>` live on the
...
platform enforces** — the box runs until _manually_ stopped, and a destroy wipes the disk with no
...
unconditionally on startup so the _identical launch command_ resumes. Cadence formula + atomic-write
...
- **GEN6 — A box reboot silently orphans the run (`tmux` does not survive it).** Symptom: a detached
  ...
  resume idempotent (§4) so the _same_ launch command continues from the last checkpoint; for a box that
...
- **GEN7 — A second concurrent run silently halves throughput by oversubscribing the GPU.** Symptom: two
  ...
  contend for VRAM and SM time. → Fix: the operator _is_ the scheduler — serialize with the
...
- **GEN8 — Watching a poll connection, not the run, declares a false death.** Symptom: the ssh-poll
  ...
  dropped _poll_ connection ≠ the training dying; the two failure modes are conflated. → Fix: on any poll
...
The box has no console — every diagnostic is an ssh one-liner. Run these _separately_ (a kill drops the
...
# THIN DIFF — SLURM _(sbatch replaces tmux)_
...
wrap `srun` _inside_ an `sbatch` script for long runs.
...
- **SLURM1 — Checkpoint _inside_ the signal handler corrupts the checkpoint.** Symptom: `--requeue`
...
- **SLURM2 — Warning signal arrives too late; the SIGKILL lands mid-write.** Symptom: the
  ...
  full checkpoint fits _before_ the wall even with the 60 s jitter; checkpoint _periodically_ too (never
...
- **SLURM5 — `$TMPDIR` checkpoints evaporate when the job ends.** Symptom: a requeued/array job finds an
  ...
  FS persists across a requeue or a different node. → Fix: stage _scratch_ to `$TMPDIR` for speed, but
...
- **Live resource use:** `sstat -j <jobid>.batch --format=JobID,MaxRSS,MaxVMSize` on a _running_ step
...
# THIN DIFF — KUBERNETES _(a Job manifest replaces the shell)_
...
each failure creates a _new_ pod, it does not restart the old one; verified kubernetes.io Jobs doc
...
- **Teardown is two-layered:** `kubectl delete job <name>` frees the _pod_ (cheap), but the underlying
...
- **K8S1 — Pod stuck `Pending`: `Insufficient nvidia.com/gpu`.** Symptom: `kubectl get pods` shows
  ...
  _usually not_ missing hardware — the **device-plugin DaemonSet** isn't running, so no node advertises
...
- **K8S2 — `RestartPolicy: Always` is rejected on a Job.** Symptom: `kubectl apply` errors that a Job's
  ...
  (restart the _container_ in place — keeps `/checkpoints` warm) or `Never` (a fresh pod per attempt,
...
- **K8S6 — Container runs but trains on CPU (GPU never attached).** Symptom: a pod runs to completion,
  ...
  the _node_ but the container lacks the runtime/library path. → Fix: **validate `kubectl exec <p> --
...
# THIN DIFF — COLAB / KAGGLE _(not SSH-orchestratable)_
...
**checkpoint-to-cloud + idempotent resume**. Teardown is automatic and free; the _opposite_ problem to
...
- **Free tier requires the browser tab to STAY OPEN** — _(verified — corrects the draft's "anti-idle
  tricks are unreliable" framing)_: **background execution is a Pro+ paid feature**; on free tier closing
  the tab stops the runtime shortly after (verified github.com/googlecolab/colabtools#4151 + community
  reports 2026-06). So keep-alive hacks aren't merely _unreliable_ — there is **no supported headless
...
- **NB3 — Kaggle batch (commit) run picks the WRONG accelerator / has no internet.** Symptom: a committed
  ...
  GPU and Internet = On in the notebook _before_ committing; verify with `torch.cuda.is_available()` in an
...
- **NB5 — Free Colab disconnect mid-epoch with no warning.** Symptom: the session simply dies; there is
  **no SIGTERM, no grace window** to catch. → Root cause: unlike Slurm/K8s, a notebook eviction gives no
  signal — the resume contract is the _only_ defense. → Fix: checkpoint every N steps to Drive
...
- **Is the checkpoint really in Drive?** `!ls -la /content/drive/MyDrive/proj/*.pth` _after_ a
