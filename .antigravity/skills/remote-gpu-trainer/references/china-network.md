...
Swap the _source_, not the workflow. Same package names, same repo IDs — only the endpoint changes. Ship
...
`datasets` snapshot `HF_ENDPOINT` the moment they are **imported**. Setting it _after_ the import (or in a
...
4. **A real overseas proxy ONLY when a model exists _only_ on `huggingface.co`** and neither mirror nor
