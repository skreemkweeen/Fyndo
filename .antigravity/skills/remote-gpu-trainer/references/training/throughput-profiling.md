...
How to tell _why_ a rented GPU is underfed (GPU-bound vs data-bound vs comms-bound), then apply the
right speedup in cost order — from a free dataloader knob to `torch.compile` and fused attention. This
layer owns _making it RUN fast + locating the mechanical bottleneck_; **verifying-dl-experiments** owns
_is the resulting number correct_. Cross-link it (**REQUIRED**) wherever a speedup risks changing the
...
- **GPU-bound** (the good case): util high _and_ SM clock/power high (T2); adding workers doesn't help. Only
...
is GPU-bound. The _0%-util-but-running_ (CPU-data-bound) inverse is **U38**, owned by verifying-dl-experiments.
...
with a real trace (T19) before investing in a fix. **GPU SM% low while _many_ python threads thrash a few
...
_why/when_. Each helps a _different_ failure, so apply by symptom, not as a blanket cargo-cult.
...
**Symptom**: with workers on, the GPU still periodically stalls — every _Nth_ step (N = `num_workers`) has
...
batches in RAM (re-check U9). A _smoothing_ knob, not a multiplier: if the **average** fetch rate is below
...
- **CPU-transform-bound**: a heavy per-sample augment (resize/decode/FFT) saturates CPU; workers CPU-pegged
  ...
  on tensors, kornia) onto idle GPU cycles. The _0%-util_ serialized-transform variant is **U38**, owned by
...
_loses_ time. Trade-off: it is **nondeterministic** (picks by first-batch timing), so it fights the
...
read/write bandwidth — a speedup _on top of_ the memory saving (oom-memory.md M6).
...
**references/training/precision-stability.md P1–P10** (cross-link; do NOT restate). The _memory_ angle and
...
instead of silently losing speed. Whether the compiled _numbers_ match eager → verifying-dl-experiments
(**REQUIRED**).

### T16 — `torch.compile` recompilation trap: variable shapes silently blow the cache → eager

**Symptom**: a compiled run is _slower_ than eager, or stutters periodically; throughput never stabilizes.
...
- **Last resort**: raise `torch._dynamo.config.recompile_limit` only if a handful of _stable_ extra shapes
...
On the frontier, checkpoint only the _fewest/heaviest_ blocks needed to fit, not the whole model.
...
**Symptom**: need to _prove_ where time goes (which T1 case), not infer from util%.
...
**Symptom**: torch.profiler shows GPU-idle gaps but not _why_ (CPU launch latency, a hidden sync, a memcpy,
...
`scp` the `.nsys-rep` down, open in the Nsight Systems GUI. Nsight **Systems** finds _which_ kernel is slow;
Nsight **Compute** (`ncu`) finds _why_ (occupancy, bandwidth, warp stalls) — but `ncu` is heavy, reserve it
...
instantly distinguishes a _hung_ process (stuck in `recv`/lock/`all_reduce`) from a _slow_ one (busy in a
...
For _what allocated the memory_ (not time), the `torch.cuda.memory._record_memory_history` snapshot +
...
trace shows `all_reduce`/`all_gather` _not_ overlapped with backward compute.
...
(https://github.com/pytorch/pytorch/issues/67570). Overlap _breaks_ when something forces a sync: an
...
- **references/multinode.md** — inter-node NCCL/NIC/fabric/timeout/MTU (the wire between boxes). Single-box
  users skip.
- **verifying-dl-experiments** (**REQUIRED**) — owns _is-the-number-real_: whether a kernel/precision/compile
  swap changed the result, whether dropping samples or a GPU-side transform shifted the distribution, the
  0%-util diagnosis (U38), determinism (U36). This file makes training _fast_; that skill decides if the
  _faster result is still true_.
