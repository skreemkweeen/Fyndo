...
How to read a CUDA OOM trace, understand _what_ fills the card (params vs optimizer vs gradients vs
activations vs fragmentation), and apply the fixes **in cost order** — from a free batch-size cut to
ZeRO-3/QLoRA sharding. This layer owns _making training RUN and fit_; **verifying-dl-experiments** owns
_is the resulting number correct_. Cross-link it (**REQUIRED**) wherever a "fix" risks changing the
...
| **Subtotal (fixed, batch-independent)**       | **~18 B/param** | a 4 B-param model ≈ 72 GB _before any activation_                      |
...
the 6+8+4 split and the "4B params, batch 16 ≈ 85 GB" worked figure. **Why a model that _infers_ in 16 GB
...
- **Tried to allocate X** — the size of the _single_ failing request. Large X = a big tensor (long-seq
...
4. **`PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`** (M8) — free; fixes _fragmentation_ OOM only.
...
10. **LoRA / QLoRA** (M13) — for _finetuning_: freeze base, train adapters; QLoRA quantizes base to 4-bit.

Rungs 1–4 and 7 do **not** alter the model/optimization math; rung 6 does (declare it, re-verify per
verifying-dl-experiments). Rungs 8–10 change _where_ state lives, not the math (LoRA changes capacity).
...
_effective_ batch identical (`effective = micro_batch × accum × world_size`). Gradients accumulate over
...
loss-scaling and won't overflow/underflow — fewer NaN failures. Note fp16 can _increase_ memory at small
...
**Fix**: launch with `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True` (env var, set _before_ the process
...
on _large_ blocks: `max_split_size_mb:<N>` (stops the allocator splitting blocks above N MiB). This is the
...
**Symptom**: _finetuning_ a large pretrained model; full-finetune state won't fit.
...
(https://arxiv.org/abs/2305.14314) and repo (https://github.com/artidoro/qlora). Note: LoRA _changes model
capacity_ — it is a different optimization target, not a free OOM trick. Whether the LoRA result matches
...
peak is a single huge layer, gradient-checkpoint _that block_ specifically.
...
1. **Eval batch > train batch**, or no-grad eval lets a _larger_ batch be attempted that exceeds the train
...
list. Eval-artifact _sizing_ (per-sample dumps blowing up) is owned by **verifying-dl-experiments**.
...
**Root cause**: Adam **lazily allocates** its m/v state (the 8 B/param) on the _first_ `optimizer.step()`,
...
**Fix**: budget for the **post-step** peak, not step-1 — measure peak with `max_memory_allocated()` _after_
...
**Symptom**: need to know _what_ is resident before choosing a fix.
...
**Symptom**: summary stats aren't enough — need _which line of code_ allocated the memory that OOMs.
...
- `torch.cuda.empty_cache()` returns cached-but-unused blocks **to the driver**; it does **not** free live
  tensors and does **not** make more room for _your own_ process (the allocator would reuse that cache
  anyway). It only helps a _second_ process on the same GPU, or it reduces fragmentation at a ~10% speed
...
- **Rising `reserved` ≠ leak.** The caching allocator holds freed blocks for reuse; reserved climbing then
  plateauing is normal. A _true_ leak is **rising `memory_allocated()`** across steps with constant batch —
...
- Calling `empty_cache()` every step to "stay safe" just slows training and can _increase_ fragmentation.

Real OOM-mechanics leaks (accumulate-loss-tensor, no `detach`) belong here; whether a _metric_ drift is a
...
- **Is the post-fit number correct** (precision swap, seq-len change, LoRA-vs-full, accumulation loss
  weighting, determinism) → **verifying-dl-experiments** (REQUIRED — this layer makes it _fit and run_; that
  one decides if the _result is true_).
