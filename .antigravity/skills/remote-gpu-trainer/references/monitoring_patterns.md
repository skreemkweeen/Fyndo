...
1. **Foreground Bash hard-caps at 600 s (10 min).** A long foreground wait/monitor is _killed_ at the cap
...
3. **A never-_exiting_ watcher never notifies.** No exit event = no notification, ever. A persistent
...
- **Externalize transfer/monitor state to a stable OS path** + a DONE marker file _outside_ the session
...
- **One waiter per live run.** Superseding a run → STOP its old waiter _first_ (TaskStop, or dismiss a
...
- Early-stop far below baseline with a grad_norm P99 spike in epoch 1–2 → likely **probabilistic
  ...
  `verifying-dl-experiments` (REQUIRED) — this skill owns _running_ the retry, not judging the number.
...
runner _if_ it reaches the box, else to the box's own `cron`/`at` + a push (Rule 1). Bind **L3** to the
