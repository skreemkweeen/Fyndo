...
**Symptom → Root cause → Fix**. "Mixed" entries are universal in symptom but carry a _platform-specific
value_ in the fix — the rule stays here, the value lives in a profile. Platform-only gotchas (AutoDL's
...
`scp`-ing a new version mid-run makes bash seek to its saved offset in a _now-different_ file, land
mid-command, and re-execute a block (duplicate runs, stalled queue). A child invocation (`bash run_one.sh`)
IS re-read fresh for the _next_ item — but only if none is parked mid-script. (principle #6.)

**Fix**: **never overwrite a script any process is executing** — check `pgrep -af <script>` first; version
the filename for hot changes (`run_one_v2.sh`), point only _new_ launches at it. Appending lines to a queue
...
1. **Machine restart / reboot** — `dmesg` is _clean_, GPU idle, cgroup roomy, `uptime` low. Most-missed: nothing in the log hints at it.
...
**REQUIRED** for the sizing policy). The _inode-cap number_ is a profile fact (some platforms enforce a hard
...
Shrinking the problem also fixes it — a smaller matrix dim shrinks footprint _quadratically_ (dim 1024 ≈
...
to fit _alongside_ any concurrent job; set `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True` to cut
...
VRAM, but `nvidia-smi`'s process table can't attribute it — so the GPU _looks_ empty while memory is locked.
...
marker then runs eval) guarantees RESULTS but gives no _reporting cadence_ — nothing re-invokes the
...
- **Live progress (best-effort)**: a session-bound local loop (e.g. `/loop 30m` / cron `3,33 * * * *`) that ssh-polls with the _local_ key. Be honest it dies when the session closes — the remote still finishes; the user re-pings to pull.
...
**Fix**: on a _rental_ the card can't be reseated — **stop the instance and re-rent a different box**; don't
...
on the critical path. (Distinct from U38's _0%_ CPU-data-bound transform case; this is the partial-starve
knob set.)
...
metadata cost scales with file _count_, not bytes.
...
thrash (the _host scheduling_ starves the GPU, the inverse of being data-bound).
...
`torch<2.9` can _downgrade_ the only build with kernels for a new-arch card (e.g. sm_120).
...
**Fix**: `docker run --gpus all …`, NVIDIA Container Toolkit ≥1.14, and **validate `nvidia-smi` _inside_ the
...
**Root cause**: bare-SDK offline mode needs no key, but project logger _wrappers_ often probe the API
...
first log lines show `Syncing run <name>` + a run URL — treat the _absence_ of that line as a failure. Run
...
**Fix** (the rule is universal; the _value_ is per-profile): (1) **align the path** — point your logger at
