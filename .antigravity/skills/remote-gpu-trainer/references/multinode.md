...
_how the process group forms and breaks_, never the resume mechanism.
...
- **REQUIRED:** treat the restored run as a _resume of the identical config_, never a hand-patched relaunch — a
...
For fanning a _sweep_ across nodes (independent cells, not one job over many nodes), that is
