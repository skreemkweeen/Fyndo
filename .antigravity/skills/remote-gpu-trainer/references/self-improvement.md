...
If it fails the bar it is a _project note_ (→ §4 memory), not a catalog entry. Enshrining unverified
...
Store as a `project` / `reference` memory. Record only the credential's _name or path_,
