...
`/ss-review` tells you _what's wrong_. `/ss-score` tells you _how good it is
overall_ and _what to fix first_ — a single number plus a category breakdown, so
...
each axis that is _mixed_ rather than unified across the file: mixed radius
...
- As a _gate_, ≥ 80 is a floor before showing the user — but don't over-polish: chasing 95→100
